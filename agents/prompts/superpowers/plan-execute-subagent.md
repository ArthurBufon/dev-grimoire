...
Este prompt não apenas ativa o workflow de `subagent-driven-development`; ele também deixa claro **qual comportamento o agente deve seguir durante a execução**: implementar uma task por vez, não alterar escopo, aplicar KISS, respeitar regras do projeto e revisar cada entrega antes de avançar. Isso reduz improviso, drift de escopo e refatorações desnecessárias.
