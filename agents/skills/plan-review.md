...
Use esta skill para revisar uma task, spec ou plano antes da execução.

O usuário fornecerá a task/plano na mensagem atual.
...
* questione decisões;
* pressione ambiguidades;
* identifique riscos escondidos;
* force clareza antes da execução;
* encontre decisões implícitas;
* proponha decisões padrão quando possível.
...
Quando algo puder ser inferido pelo próprio plano, contexto fornecido ou padrões comuns do projeto, não pergunte: infira, justifique e aponte a suposição.
