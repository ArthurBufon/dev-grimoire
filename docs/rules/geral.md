...
* O diretório `docs/` do projeto deve conter somente specs de features (`docs/features/`) e contexto de negócio — convenções ficam no **Dev Grimoire** local (clone `dev-grimoire` no pai, ancestral ou pasta irmã do ancestral; ver `docs/rules/global.md`)
